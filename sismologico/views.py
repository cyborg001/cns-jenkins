...
def ejecucion_horaria(segundos):
    ...
    while True:
        r = requests.get('http://127.0.0.1:8000/uploader').json()
        print(r)
        if r.get('error') :
            print(r['error'])
            r = None
            ...
            if validar_fecha(data['fecha'],data['hora']):
                ...
                sismo = Sismo(user=analista,custom_id=data['id'],
                ...
                magW=data['magW'],gapInfo=data['gapInfo'],focalInfo=data['focalInfo'],
...
# hilo = threading.Thread(target=ejecucion_horaria, args=(20,))
# hilo.daemon =  True
